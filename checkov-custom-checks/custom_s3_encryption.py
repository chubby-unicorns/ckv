...
class BucketEncryption(BaseResourceCheck):
    def __init__(self):
        ...
        We have 4 Requirements for S3 buckets:
            ...
            [2] All buckets containing PII data must be encrypted using KMS (buckets to be idenifiable via tag, e.g. "PII")
            [3] If a bucket is configured to use KMS, it must have a KMSMasterKeyID specified and 
            ...
            if "BucketEncryption" in Properties.keys():
                ...
                if "ServerSideEncryptionConfiguration" in BucketEncr.keys():
                    ...
                    if is_pii and is_kms and has_master_key and has_bucket_key:
                        # If PII data it must use kms, have a key configured and that be a bucket key
                        CheckResult.PASSED
                    elif is_kms and has_master_key and has_bucket_key:
                        # It's OK to use KMS, as long as it has a master key configured and bucketkey enabled
                        CheckResult.PASSED
                    elif not is_pii and not is_kms:
                        # Fallback is SSES3 (AES256) - It's OK to use AES256, as long as it is not PII data
                        CheckResult.PASSED
                    else:
